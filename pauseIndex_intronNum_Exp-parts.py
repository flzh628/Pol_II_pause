...
parser = argparse.ArgumentParser(description='To analyze the relationship between pauseIndex and IntronNum, at different expression levels.',
                                 epilog='Author: Fengli Zhao\tzhaofl@sustech.edu.cn      20220426 20220425 20211111 20210912 20210908')

parser.add_argument('--gpart', type=str, required=True, help='The Exp-part of gene.')
...
df.to_csv(prefix + '.PauseIndex_IntronNum_Exp-parts.data', sep='\t', index=False)

outstat = prefix + '.PauseIndex_IntronNum_Exp-parts.statistics'
if os.path.exists(outstat):
    os.remove(outstat)

CORR = open(prefix + '.PauseIndex_IntronNum_Exp-parts.corr', 'w')
...
for part in parts:
    ...
    MultiComp.OneFactor_multiComparison(df_p, 'Intron_Num', 'Index', 'Fix_GeneTPM_' + part, outstat)
...
boxplot_fig.save(prefix + '.PauseIndex_IntronNum_Exp-parts.boxplot.pdf', width=4.8, height=9, format='pdf')
